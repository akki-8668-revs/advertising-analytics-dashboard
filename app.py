...
from config import get_google_credentials, get_data_from_google_sheets
...
def load_data_from_drive():
    """Load data from CSV files in Google Drive folder"""
    try:
        import requests
        import io

        # Get Google Drive folder URL from secrets
        drive_folder_url = st.secrets.get("GOOGLE_DRIVE_FOLDER_URL", "")

        if not drive_folder_url:
            st.error("Google Drive folder URL not configured")
            return None, None

        # PLA Data from CSV
        pla_csv_url = st.secrets.get("PLA_CSV_URL", "")
        pla_df = None
        if pla_csv_url:
            try:
                st.info("Loading PLA data from Google Drive...")
                response = requests.get(pla_csv_url)
                if response.status_code == 200:
                    pla_df = pd.read_csv(io.StringIO(response.text))
                    # Convert date column
                    if 'day_date' in pla_df.columns:
                        pla_df['day_date'] = pd.to_datetime(pla_df['day_date'], errors='coerce')
                    ...
                    for col in numeric_cols:
                        if col in pla_df.columns:
                            pla_df[col] = pd.to_numeric(pla_df[col], errors='coerce').fillna(0)
                    st.success(f"PLA data loaded: {len(pla_df):,} rows")
                else:
                    st.warning("Could not download PLA CSV file")
            except Exception as e:
                st.warning(f"Error loading PLA data: {e}")

        # PCA Data from CSV
        pca_csv_url = st.secrets.get("PCA_CSV_URL", "")
        pca_df = None
        if pca_csv_url:
            try:
                st.info("Loading PCA data from Google Drive...")
                response = requests.get(pca_csv_url)
                if response.status_code == 200:
                    pca_df = pd.read_csv(io.StringIO(response.text))
                    # Convert date column
                    if 'day_date' in pca_df.columns:
                        pca_df['day_date'] = pd.to_datetime(pca_df['day_date'], errors='coerce')
                    # Convert numeric columns
                    numeric_cols = ['viewcount', 'clicks', 'adspend', 'direct_units', 'indirect_units',
                                   'ppv', 'direct_rev', 'indirect_rev']
                    for col in numeric_cols:
                        if col in pca_df.columns:
                            pca_df[col] = pd.to_numeric(pca_df[col], errors='coerce').fillna(0)
                    st.success(f"PCA data loaded: {len(pca_df):,} rows")
                else:
                    st.warning("Could not download PCA CSV file")
            except Exception as e:
                st.warning(f"Error loading PCA data: {e}")
...
# Backward compatibility - try Drive first, fallback to Sheets
@st.cache_data(ttl=3600)
def load_data_from_sheets():
    """Load data from Google Sheets (fallback method)"""
    try:
        # PLA Data (Brand Level)
        pla_sheet_id = st.secrets.get("PLA_SHEET_ID", "")
        if pla_sheet_id:
            pla_data = get_data_from_google_sheets(pla_sheet_id, "PLA_Data")
            if pla_data:
                pla_df = pd.DataFrame(pla_data[1:], columns=pla_data[0])  # First row is headers
                # Convert date column
                if 'day_date' in pla_df.columns:
                    pla_df['day_date'] = pd.to_datetime(pla_df['day_date'], errors='coerce')
                # Convert numeric columns
        ...
        # PCA Data (Campaign Level)
        pca_sheet_id = st.secrets.get("PCA_SHEET_ID", "")
        if pca_sheet_id:
            pca_data = get_data_from_google_sheets(pca_sheet_id, "PCA_Data")
            if pca_data:
                pca_df = pd.DataFrame(pca_data[1:], columns=pca_data[0])  # First row is headers
                # Convert date column
                if 'day_date' in pca_df.columns:
                    pca_df['day_date'] = pd.to_datetime(pca_df['day_date'], errors='coerce')
                # Convert numeric columns
...
def create_kpi_summary(df, title, data_type='pla'):
    ...
    if df is None or df.empty:
        st.warning("No data available for KPI summary")
        return
...
def main():
    ...
    # Load data from Google Drive (primary) or Sheets (fallback)
    with st.spinner("Loading data from Google Drive..."):
        pla_df, pca_df = load_data_from_drive()

        # Fallback to Google Sheets if Drive fails
        if (pla_df is None and pca_df is None):
            st.warning("Trying Google Sheets as fallback...")
            pla_df, pca_df = load_data_from_sheets()

    if pla_df is None and pca_df is None:
        st.error("Unable to load data from Google Sheets. Please check your configuration.")
        st.stop()
    ...
    data_sources = []
    if pla_df is not None:
        data_sources.append("PLA (Brand Level)")
    if pca_df is not None:
        data_sources.append("PCA (Campaign Level)")
    ...
    if df is None or df.empty:
        st.error(f"No data available for {data_source}")
        st.stop()

    # Calculate KPIs
    df_with_kpis = calculate_pla_kpis(df) if data_type == 'pla' else calculate_pca_kpis(df)

    if df_with_kpis is None:
        st.error("Failed to calculate KPIs")
        st.stop()

    # Date filter
    if 'day_date' in df_with_kpis.columns and len(df_with_kpis['day_date'].unique()) > 1:
        ...
        if st.button("🚀 Optimize Budget Allocation"):
            with st.spinner("Analyzing historical performance and optimizing budget allocation..."):
                ...
                if optimization_results is None:
                    st.error("Failed to generate budget optimization results")
                    st.stop()
    ...
    with tab4:
        ...
        total_spend = df_filtered[spend_col].sum()
        ...
        st.info(f"📊 **Data Source**: {data_source}\n\n"
                f"📅 **Date Range**: {df_filtered['day_date'].min().strftime('%Y-%m-%d') if 'day_date' in df_filtered.columns else 'N/A'} to {df_filtered['day_date'].max().strftime('%Y-%m-%d') if 'day_date' in df_filtered.columns else 'N/A'}\n\n"
                f"📈 **Total Records**: {len(df_filtered):,}\n\n"
                f"🏷️ **Brands**: {df_filtered['brand'].nunique() if 'brand' in df_filtered.columns else 'N/A'}\n\n"
                f"📂 **Categories**: {df_filtered['analytic_super_category' if data_type == 'pla' else 'super_category'].nunique() if ('analytic_super_category' in df_filtered.columns or 'super_category' in df_filtered.columns) else 'N/A'}")
