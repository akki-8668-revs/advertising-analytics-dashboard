...
@st.cache_data
def load_pla_data():
    """Load PLA (Performance by Listing Ads) data"""
    try:
        df = pd.read_csv('pla_onetim_2026-02-26.csv')
        # Convert date column
...
@st.cache_data
def load_pca_data():
    """Load PCA (Product Creative Ads) data"""
    try:
        df = pd.read_csv('pca_onetim_2026-02-26.csv')
        # Convert date column
        df['day_date'] = pd.to_datetime(df['day_date'], format='%d-%m-%Y')
        # Convert numeric columns
...
def get_google_credentials():
    """Get Google API credentials from JSON key file"""
    try:
        import os
        json_key_path = r"C:\Users\akshay.kumar17\Documents\Secret\akshay_kumar_ads.json"

        if not os.path.exists(json_key_path):
            st.error(f"Google credentials file not found at: {json_key_path}")
            return None
...
def export_to_google_sheets(df, sheet_name="Advertising Analysis", spreadsheet_id=None):
    """Export analysis results to Google Sheets"""
    if not GOOGLE_API_AVAILABLE:
        st.error("Google API libraries not available. Install with: pip install google-auth google-api-python-client gspread")
        return None
    ...
    try:
        ...
        st.success(f"✅ Data exported to Google Sheets: {sh.url}")
        return sh.url
...
def export_insights_to_google_docs(insights, recommendations, doc_title="Advertising Performance Insights"):
    """Export insights and recommendations to Google Docs"""
    if not GOOGLE_API_AVAILABLE:
        st.error("Google API libraries not available. Install with: pip install google-auth google-api-python-client gspread")
        return None
    ...
    try:
        ...
        doc = service.documents().create(body=doc_request).execute()
        ...
        requests.append({
            'insertText': {
                'location': {'index': requests[-1]['insertText']['location']['index'] + len(requests[-1]['insertText']['text'])},
        ...
        st.success(f"✅ Insights exported to Google Docs: {doc_url}")
        return doc_url
    ...
    with tab4:
        ...
        with col2:
            if st.button("📈 Export to Google Sheets"):
                spreadsheet_url = export_to_google_sheets(df_filtered, f"Advertising Analysis - {data_source}")
                if spreadsheet_url:
                    st.markdown(f"[Open in Google Sheets]({spreadsheet_url})")

        with col3:
            if st.button("📝 Export Insights to Google Docs"):
                ...
                if avg_roi > 2.0:
                    insights_for_export.append("✅ Excellent ROI performance - you're generating strong returns on ad spend")
                elif avg_roi > 1.0:
                    insights_for_export.append("⚠️ Moderate ROI - there's room for optimization")
                else:
                    insights_for_export.append("❌ Poor ROI - immediate attention needed to improve campaign efficiency")

                if avg_ctr > 0.02:
                    insights_for_export.append("🎯 Strong CTR indicates good ad relevance and targeting")
                elif avg_ctr > 0.01:
                    insights_for_export.append("📊 Average CTR - consider A/B testing ad creative")
                else:
                    insights_for_export.append("📉 Low CTR - review ad creative and targeting strategy")

                direct_revenue_pct = (df_filtered['direct_rev'].sum() / total_revenue * 100) if total_revenue > 0 else 0
                if direct_revenue_pct > 70:
                    insights_for_export.append("💪 Strong direct revenue contribution - excellent conversion optimization")
                elif direct_revenue_pct > 50:
                    insights_for_export.append("🔄 Balanced direct/indirect revenue mix")
                else:
                    insights_for_export.append("🔗 Heavy reliance on indirect revenue - focus on direct conversion optimization")

                doc_url = export_insights_to_google_docs(insights_for_export, recommendations, f"Advertising Insights - {data_source}")
                if doc_url:
